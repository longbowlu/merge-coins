...
async fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        let gas_obj = sui_client.read_api().get_object_with_options(gas, SuiObjectDataOptions::new()).await.unwrap().data.unwrap().object_ref();
        let coins = stream
            .take(512)
            .map(|coin| {
        ...
        let rebate = resp.effects.unwrap().gas_cost_summary().net_gas_usage();
